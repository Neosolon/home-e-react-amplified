...
export class Portfolio {
  ...
  getStockActionTotal() {
    return this.stockActions.reduce(
      (acc, stockAction) => (acc += stockAction.finalAmount),
      0
    )
  }
  ...
  getStockAllocations(investment) {
    ...
    if (Math.abs(totalAllocation - investment) > 0.001) {
      throw new Error(
        `Error calculating target allocations ${totalAllocation} ${investment}`
      )
    }
...
export function balancePortfolios(portfolios) {
  ...
  const totalPortfolioValue = portfolios.reduce(
    (total, portfolio) => total + portfolio.total,
    0
  )
  const totalValue = stockValues.reduce(
    (total, stockValue) => total + Number(stockValue.currentPosition),
    0
  )
  ...
  portfolios.forEach((portfolio, index) => {
    const stock_plan = portfolio.rebalance_stocks(
      totalInvestment * portfolio.ratio
    )
